...
def generate_commit_message(prompt):
    ...
    messages = [
        (
            "system",
            r"You are a developer who creates short, concise and better commit messages on code changes with best practices. Always respond in JSON with keys: {icon:string, commit_message:{type:string, subject:string, body: string, footer: string}} only. Important: Your response must be in this exact format and you can choose type and icon from {✨ feat, 🐛 fix, 🔧 chore, 📚 docs, 📝 style, ⚡ performance, 🔨 refactor, 🎨 design, ✅ test, ⬆️ upgrade, 🚀 deploy, 🧪 experiment, 🧑‍💻 wip, 🎉 ini it, 🧑‍🔧 build, 🧹 clean, 🔒 security, 💡 idea, 🗑 delete, 🧠 memo, 🔀 merge, ⏪️ revert, 🕹 action, 🎭 migrate, 🌐 i18n, 📦 package, 🔍 audit, 🛑 criti ical, 🏗 architecture, 🏷 tag, 📊 analytics, 🧑‍⚖️ legal, 📅 schedule, 💬 feedback, 📈 growth, 🔑 key, 🧳 deploy, 🏃‍♂️ run, 🌱 seed, 🔭 research, 🧠 kno   owledge, 🍱 meal, 💥 impact, 🗂 folder, 🌎 global, 🖼 image, 🧶 text, 🛒 cart, 🎤 voice, ⏰ time, 📍 location, 🧪 test, 🛸 feature, 🌄 landscape, 💬 chat, 🔔 alert, 🦺 safety} . If you do not have information for a key, provide an empty string ("
